...
def list_hot_forecast_combinations(
    context: WorkerRuntimeContext, *, limit: int = 50
) -> list[dict[str, Any]]:
    if context.mode != "attached" or not context.runtime_bundle:
        return []

    callbacks = context.runtime_bundle.get("callbacks", {})
    endpoints = callbacks.get("endpoints", {})
    path = str(endpoints.get("forecastCombinationPath", "")).strip()
    if not path:
        return []

    if path.startswith(_base_url()):
        request_path = path[len(_base_url()) :]
    else:
        request_path = path

    separator = "&" if "?" in request_path else "?"
    payload = _request("GET", f"{request_path}{separator}limit={limit}")
    combinations = payload.get("combinations")
    if not isinstance(combinations, list):
        return []

    return [entry for entry in combinations if isinstance(entry, dict)]
